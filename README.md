...
1. 配置 Stripe API 金鑰（見上文）
2. 用戶在結帳頁面選擇 Stripe 付款
3. 在嵌入的表單中輸入信用卡資訊
4. 付款成功後自動批准訂單並執行 RCON 指令發貨
5. **注意**：最低金額為 NT$20（由於 Stripe 匯率轉換限制）
...
4. **定期更新 Docker 鏡像**
5. **不要將敏感配置文件提交到版本控制**：
   - ⚠️ `api/paypal-config.json`
   - ⚠️ `api/stripe-config.json`
   - ✅ 已加入 `.gitignore`
6. **切換到生產環境時**：
   - PayPal: 將 `mode` 改為 `"live"` 並使用正式環境金鑰
   - Stripe: 使用 `sk_live_` 和 `pk_live_` 開頭的金鑰
